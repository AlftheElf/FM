...
--Changelog:
-- Added conditional to test if mage is offline, and doesn't add to the FM list if so.
	...
	if (  GetNumRaidMembers() > 0 ) then
		...
		if (GetNumRaidMembers() == numRaidMembers) then -- If raid members changed during function
			...
			for i = 1, #FM_name, 1 do
				if ( FM_class[i] == 'MAGE' and UnitIsConnected(FM_name[i]) ) then
